...
<div class="button">
	...
	<a href="{{ route(SharedData::get('route').'.postDelete') }}" class="btn btn-danger">
		{{ __('Delete') }}
	</a>
				...
				<tbody>
					...
					<tr>
						...
						<td class="text-center">
							<input type="text" name="sort[]" class="form-control form-control-sm text-center sort"
								key="{{ $table->field_primary }}" value="{{ $table->field_sort }}">
						</td>
						<td class="text-center">
							<btn
								class="badge badge-{{ $table->field_enable == BooleanType::Yes ? 'success' : 'warning' }}">
								{{ BooleanType::getDescription($table->field_enable) }}
							</btn>
						</td>
						<td class="text-center">
